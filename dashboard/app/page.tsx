...
export default function OverviewPage() {
  ...
  return (
    <div className="space-y-8">
      {/* Non-dismissible Case Study Disclaimer Banner */}
      <div className="bg-amber-500/10 border border-amber-500/30 rounded-xl p-4 flex items-start gap-3 text-amber-300 text-sm">
        ...
        <div>
          <h4 className="font-semibold text-amber-200">Educational Simulation & Non-Affiliation Disclaimer</h4>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-800 pb-5">
        <div>
          <h2 className="text-2xl sm:text-3xl font-extrabold text-slate-100 tracking-tight">
            Delivery Partner Business Overview
          </h2>
          <p className="text-sm text-slate-400 mt-1">
            Rawalpindi / Islamabad Delivery Partner Performance Case Study & 3-Month Demand Outlook
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
        ...
        <div className="bg-slate-900/90 border border-slate-800 hover:border-slate-700 transition-all rounded-xl p-5 space-y-3 relative overflow-hidden group">
          <div className="absolute -right-6 -bottom-6 w-24 h-24 bg-indigo-500/5 rounded-full group-hover:scale-125 transition-transform" />
        ...
        <div className="bg-slate-900/90 border border-slate-800 hover:border-slate-700 transition-all rounded-xl p-5 space-y-3 relative overflow-hidden group">
          <div
            className={`absolute -right-6 -bottom-6 w-24 h-24 rounded-full group-hover:scale-125 transition-transform ${
              isGrowthPositive ? "bg-indigo-500/5" : "bg-rose-500/5"
            }`}
          />
          <div className="flex items-center justify-between">
            <span className="text-xs font-semibold uppercase tracking-wider text-slate-400">
            ...
            <div
              className={`p-2 rounded-lg ${
                isGrowthPositive ? "bg-indigo-500/10 text-indigo-400" : "bg-rose-500/10 text-rose-400"
              }`}
            >
            ...
            <div className="flex items-baseline gap-2">
              <div
                className={`text-3xl font-black ${
                  isGrowthPositive ? "text-indigo-400" : "text-rose-400"
                }`}
              >
        ...
        <div className="bg-gradient-to-br from-slate-900 via-slate-900 to-indigo-950/30 border border-indigo-500/30 rounded-xl p-5 space-y-3 relative overflow-hidden group">
          <div className="flex items-center justify-between">
          ...
          <div>
            <div className="text-3xl font-black text-indigo-300">
              ...
              <span className="text-sm font-normal text-indigo-400/80">orders</span>
            </div>
            <p className="text-xs text-indigo-400/90 mt-1">
              Est. Revenue: <strong className="text-indigo-200">PKR {Math.round(kpis.nextMonthForecastRevenue).toLocaleString()}</strong>
            </p>
          </div>
        </div>
      ...
      <div className="bg-slate-900/90 border border-slate-800 rounded-xl p-6 space-y-6">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          ...
          <Link
            href="/forecast"
            className="inline-flex items-center gap-2 text-xs font-semibold text-indigo-400 hover:text-indigo-300 bg-indigo-500/10 hover:bg-indigo-500/20 border border-indigo-500/30 px-3.5 py-2 rounded-lg transition-all self-start sm:self-auto"
          >
